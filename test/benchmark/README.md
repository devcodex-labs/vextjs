...
当前可引用的主对照测量结果只在 [`RESULTS.md`](./RESULTS.md) 中。该文件由 runner 自动生成，并记录源码 SHA、worktree 状态、Node/OS/CPU/内存、锁定依赖版本、Autocannon 版本、npm latest 校验时间、完整参数、每轮样本和 CV；不要把本 README 中的示例或其他机器的数值当作当前基准。两个 runner 会在启动压测前查询 npm registry；任一对照框架或 Autocannon 不是 `latest`，或 manifest、lock、实际安装树不一致时都会直接失败。Autocannon 作为精确 devDependency 通过官方 programmatic API 运行，避免每个样本重复执行 `npm exec` 引入解析/网络/缓存噪声。也可单独运行 `npm run verify:benchmark-deps`。
...
node --expose-gc --max-old-space-size=512 test/benchmark/run-native-fairness.mjs --scenario all --duration 10 --connections 50 --pipelining 10 --warmup 5 --rounds 5 --max-cv 15 --process-priority 0 --handler-mode async
...
# 长测可按场景生成 complete artifact；只合并同源码、同依赖、同环境、同协议且 CV 过门禁的四个场景
node --expose-gc --max-old-space-size=512 test/benchmark/run-native-fairness.mjs --scenario json --duration 10 --connections 50 --pipelining 10 --warmup 5 --rounds 5 --max-cv 15 --handler-mode sync --results-json ./test/benchmark/.artifacts/sync-json.json --output ./test/benchmark/.artifacts/sync-json.md
node test/benchmark/run-native-fairness.mjs --from-results-json ./test/benchmark/.artifacts/sync-json.json,./test/benchmark/.artifacts/sync-params.json,./test/benchmark/.artifacts/sync-chain.json,./test/benchmark/.artifacts/sync-middleware-chain.json --require-complete-matrix --output test/benchmark/RESULTS.md
```

主 runner 会在写报告前断言：Core 的 global middleware 为 `0` 且参测 route registration chain 为 `1`；Normal 在 `requestContext=false` 与 `frontend.enabled=false` 后 global middleware 为 `1`（仅 `requestHook`；`authContext` 和 frontend noop 不注册），普通 route registration chain 为 `2`（`routeMatched + handler`），`middleware-chain` 为 `5`。每轮会轮转 Raw Native / Raw Fastify / Core / Normal 的起始目标；任一多轮样本 CV 超过 `--max-cv`（默认 15%）时只保留 `complete=false` 的原始 JSON，并拒绝生成可引用 Markdown 报告。

Raw Fastify 的三层 hook 只附着在 `/middleware-chain` 路由，不会穿过 `/json`、`/users/:id`、`/chain`；Raw Native / Raw Fastify 都使用预序列化 JSON body。Core 是 `test/benchmark` 的私有 direct harness，刻意绕过 bootstrap/router-loader；它表示最短 Vext Native 路径，不表示完整生产配置。Core 设计上不注册 route middleware chain，因此该单元格是 `N/A`，不是漏测或零成本。
...
node --expose-gc --max-old-space-size=512 test/benchmark/run-benchmark.mjs --framework native,hono,fastify,express,koa --scenario all --duration 10 --connections 50 --pipelining 10 --warmup 5 --rounds 5 --max-cv 15 --process-priority 0
...
node --expose-gc --max-old-space-size=512 test/benchmark/run-benchmark.mjs --framework native --scenario json --duration 10 --connections 50 --pipelining 10 --warmup 5 --rounds 5 --results-json ./test/benchmark/.artifacts/native-json.json --output ./test/benchmark/.artifacts/native-json.md
node --expose-gc --max-old-space-size=512 test/benchmark/run-benchmark.mjs --framework native --scenario params --duration 10 --connections 50 --pipelining 10 --warmup 5 --rounds 5 --results-json ./test/benchmark/.artifacts/native-params.json --output ./test/benchmark/.artifacts/native-params.md
node test/benchmark/run-benchmark.mjs --from-results-json ./test/benchmark/.artifacts/native-json.json,./test/benchmark/.artifacts/native-params.json --output test/benchmark/RESULTS.md
...
| `--max-cv <percent>`          | `15`                        | 多轮 RPS 的最大变异系数；任一结果超出时拒绝生成可引用报告                                      |
...
当 `--rounds` ≥ 2 时，每项测试会运行指定轮次，取 **RPS 中位数**作为最终结果。Native 主对照会轮转四个目标的每轮起始位置；adapter matrix 会在每轮交换 Raw/Vext 先后顺序，避免把时间漂移固定压在某一方。CV 门禁在报告写入前执行：
...
Raw Hono 按 Hono 官方 Node.js 方式使用 `@hono/node-server`；Vext Hono adapter 运行时只依赖 `hono`，通过 Vext 自有 `node:http` bridge 接入。两者都是真实公开路径，但 server wrapper 不相同，因此 Hono 差距包含 bridge 实现成本，不能解释为同一 wrapper 下的纯路由排名。
