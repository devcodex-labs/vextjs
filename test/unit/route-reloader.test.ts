...
describe("reloadRoutes", () => {
  ...
  describe("路由加载", () => {
    ...
    it("应将 middlewareDefs 传递给 loadRoutes", async () => {
      ...
      const callArgs = loadRoutesFn.mock.calls[0]
      expect(callArgs[2].middlewareDefs).toBe(mwDefs)
    })
    ...
    it("应将 globalMiddlewares 传递给 loadRoutes", async () => {
      ...
      const callArgs = loadRoutesFn.mock.calls[0]
      expect(callArgs[2].globalMiddlewares).toBe(globalMws)
    })
  ...
  describe("中间件注册表传递", () => {
    ...
    it("应支持 Map<string, VextMiddleware> 类型", async () => {
      ...
      const callArgs = loadRoutesFn.mock.calls[0]
      expect(callArgs[2].middlewareDefs).toBe(mwMap)
    })
  })
