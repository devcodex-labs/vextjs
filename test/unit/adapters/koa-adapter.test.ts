...
import type { KoaAdapterOptions } from "../../../src/adapters/koa/adapter.js";
import type {
  VextAdapter,
  VextServerHandle,
...
describe("Koa Adapter — VextAdapter 接口合规性", () => {
  ...
  beforeEach(() => {
    ...
    adapter = createKoaAdapter({}, mockApp);
    handle = null;
  });
  ...
  describe("路由注册与响应", () => {
    ...
    it("应支持所有 HTTP 方法（PUT / PATCH / DELETE / OPTIONS）", async () => {
      const methods = ["PUT", "PATCH", "DELETE", "OPTIONS"];

      for (const method of methods) {
        const localAdapter = createKoaAdapter({}, mockApp);
    ...
    it("多段路由参数应正确解析", async () => {
      adapter.registerRoute("GET", "/posts/:postId/comments/:commentId", [
        async (req: VextRequest, res: VextResponse) => {
          res.json({
      ...
      const response = await httpRequest({
        port: handle.port,
        path: "/posts/10/comments/55",
      });

      expect(response.status).toBe(200);
      const body = JSON.parse(response.body);
      expect(body).toEqual({ postId: "10", commentId: "55" });
    });
  ...
  describe("404 Not Found", () => {
    ...
    it("未注册 notFoundHandler 时应返回默认 404 JSON", async () => {
      adapter.registerRoute("GET", "/exists", [
        async (req, res) => {
          res.json({ ok: true });
        },
  ...
  describe("VextRequest 字段", () => {
    ...
    it("trustProxy = true 时 req.ip 应从 X-Forwarded-For 读取", async () => {
      ...
      const trustedAdapter = createKoaAdapter({}, trustedApp);
      let ip = "";
  ...
  describe("buildHandler", () => {
    ...
    it("通过 buildHandler 返回的 handler 应能处理请求", async () => {
      ...
      const buildAdapter = createKoaAdapter({}, mockApp);
      ...
      // Koa app.callback() 返回标准 handler，无需先 listen
  ...
  describe("Koa 路由匹配器", () => {
    it("静态路径应正确匹配", async () => {
      adapter.registerRoute("GET", "/api/v1/health", [
        async (req, res) => {
          res.json({ status: "ok" });
        },
      ]);
    ...
    it("不匹配的方法应返回 404", async () => {
      adapter.registerNotFound(async (req, res) => {
        res.rawJson({ code: 404, message: "Not Found" }, 404);
      });

      adapter.registerRoute("POST", "/only-post", [
        async (req, res) => {
          res.json({ ok: true });
        },
      ]);
    ...
    it("多个路由应按注册顺序优先匹配（先注册先匹配）", async () => {
      adapter.registerRoute("GET", "/items/:id", [
        async (req, res) => {
          res.json({ matched: "param", id: req.params.id });
        },
      ]);

      // 这条路由在 /items/:id 之后注册，不应被匹配
      adapter.registerRoute("GET", "/items/special", [
        async (req, res) => {
          res.json({ matched: "static" });
        },
      ]);
      ...
      const body = JSON.parse(response.body);
      // 由于 :id 先注册，应优先匹配
      expect(body.matched).toBe("param");
      expect(body.id).toBe("special");
    });
  });
