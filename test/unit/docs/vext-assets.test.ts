...
describe("Vext docs vanilla assets", () => {
  it("loads OpenAPI and Code JSDoc data without React or innerHTML", () => {
    expect(VEXT_DOCS_APP_JS).toContain("fetch(config.endpoints.openapi");
    expect(VEXT_DOCS_APP_JS).toContain("fetch(config.endpoints.code");
    ...
    expect(VEXT_DOCS_APP_JS).toContain("Manual header rows or raw JSON override auto auth headers");
    ...
    expect(VEXT_DOCS_APP_JS).toContain('document.addEventListener("pointermove"');
    ...
    expect(VEXT_DOCS_STYLE_CSS).toContain("var(--vext-docs-sidebar-width, 280px) 8px minmax(0, 1fr)");
    ...
    expect(VEXT_DOCS_STYLE_CSS).toContain("grid-template-columns: minmax(0, 1fr)");
    ...
    expect(VEXT_DOCS_APP_JS).not.toContain("appendBadge(meta, getOperationKind");
    ...
    expect(VEXT_DOCS_APP_JS).not.toContain('details.classList.add("is-sending")');
    ...
    expect(VEXT_DOCS_STYLE_CSS).toContain("max-height: 54vh");
    ...
    expect(VEXT_DOCS_APP_JS).not.toContain("for (const section of groupOperations(operations, state.view))");
