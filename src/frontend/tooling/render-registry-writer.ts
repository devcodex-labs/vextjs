...
${renderSharedBrowserRendererHelpers(config.i18n.defaultLocale)}
${devRuntime}
...
async function mountVextBrowser(root) {
  ...
  const tree = await createVextTree(payload.page, payload.props ?? {}, payload.options ?? {});
  ${mountCall}
...
function renderSharedBrowserRendererHelpers(defaultLocale: string): string {
  return `async function createVextTree(page, props, options) {
...
async function resolveI18nContext(options) {
  const allMessages = {};
  for (const [locale, loader] of Object.entries(localeModules)) {
    const module = await loader();
    allMessages[locale] = module.default ?? module.messages ?? module;
  }
  const defaultLocale = ${JSON.stringify(defaultLocale)};
