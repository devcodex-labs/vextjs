...
- the host is Linux x64, the qualifying pilot freezes its current-LTS Node.js major, and the runner verifies the runner and every target's effective CPU affinity;
...
The full sample is generated into the result block on this page in both languages. It is intentionally never replaced by a GitHub-only link or a separate results page.

## Reproduce

Local runs are implementation checks only:
...
After a qualified Linux x64 pilot has been reviewed and both its Node.js major and CV gate are explicitly frozen in `test/benchmark/enterprise/protocols/linux-x64-v1.json`, run the formal suite with isolated CPU sets:
...
The generator rejects local, pilot, dirty-source, incomplete, unstable, or non-Linux artifacts. That prevents a convenient local number from silently becoming documentation evidence.
