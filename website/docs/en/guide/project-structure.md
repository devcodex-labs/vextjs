...
### `src/client/` — Frontend directory

The default full-stack scaffold creates `src/client/` for the browser app. Vext currently ships a React-first template and keeps the public frontend contract in `vextjs/frontend`.

| File | Purpose |
| ------------------ | ---------------------------------------------- |
| `main.tsx` / `main.jsx` | Browser entry used by the frontend bundler |
| `App.tsx` / `App.jsx` | Default React application |
...
- `vext start` serves the production client and applies SPA fallback outside API/documentation paths
