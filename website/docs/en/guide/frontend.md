...
| `frontend.build.vendorChunks.enabled`      | `true`                                              | Generates the Vext-managed vendor entry so React and other shared dependencies can become shared chunks through esbuild splitting.                                 |
...
| `frontend.deploy.integrity`                | `false`                                             | Whether to inject build-time SRI into JS/CSS `<script>` and `<link>` tags.                                                                                         |
| `frontend.deploy.upload.enabled`           | `false`                                             | Enables built-in static asset upload. Plain `vext build` does not upload by default; enable it explicitly or use the deploy CLI.                                   |
| `frontend.deploy.upload.adapter`           | `"filesystem"`                                      | Upload adapter. The first version includes `filesystem` and `mock`; S3, OSS, and Cloudflare can be added later through custom adapters.                            |
| `frontend.deploy.upload.targetDir`         | `".vext/deploy/frontend-assets"`                    | Directory written by the `filesystem` adapter.                                                                                                                     |
| `frontend.deploy.upload.publicBaseUrl`     | `undefined`                                         | Public URL prefix for uploaded assets. Falls back to `frontend.deploy.assetBaseUrl` when omitted.                                                                  |
| `frontend.deploy.upload.prefix`            | `""`                                                | Upload key prefix, such as `my-app/v1`.                                                                                                                            |
| `frontend.deploy.upload.stateFile`         | `".vext/deploy/frontend-assets-state.json"`         | Incremental upload state file; unchanged assets are skipped by comparing `uploadKey -> sha256`.                                                                    |
| `frontend.deploy.upload.dryRun`            | `false`                                             | Prints the upload plan without writing to the target.                                                                                                              |
| `frontend.deploy.upload.concurrency`       | `4`                                                 | Upload concurrency.                                                                                                                                                |
| `frontend.deploy.upload.include`           | `["**/*"]`                                          | Glob patterns included in the deploy manifest and upload plan.                                                                                                     |
| `frontend.deploy.upload.exclude`           | `["**/*.map"]`                                      | Glob patterns excluded from the deploy manifest and upload plan. Source maps are not uploaded by default.                                                          |
...
1. Run `npm run build`.
2. Inspect `dist/client/deploy-manifest.json` and confirm JS, CSS, images, fonts, and `public/**` resources are listed.
3. Run `npx vext deploy assets` to upload static resources. The first upload writes assets to the target, and the next upload skips unchanged files by comparing sha256 with the configured `stateFile`.
4. Use `npx vext build --upload-assets` when build and upload should run as one command. Use `--deploy-dry-run` or `vext deploy assets --dry-run` when you only need the upload plan.
5. Set `frontend.deploy.assetBaseUrl` in production config, for example `https://cdn.example.com/my-app/`.
6. Vext still renders HTML, SSR content, and page data from the server; JS, CSS, images, fonts, and other static assets load from the CDN.

The first version includes the `filesystem` adapter, which is useful for local publish folders, Nginx static roots, CI artifact folders, or a staging directory before syncing to object storage. The `mock` adapter is for tests. S3, OSS, Cloudflare R2, and other cloud providers can be added later through custom deploy adapters; Vext core does not make any vendor SDK a default dependency.

```bash
# Build, then upload
npx vext build --upload-assets

# Print the upload plan without writing to the target
npx vext deploy assets --dry-run

# Override the filesystem target and upload key prefix for this run
npx vext deploy assets \
  --target-dir .deploy/cdn \
  --prefix my-app/v1 \
  --state-file .vext/deploy/frontend-assets-state.json
```
