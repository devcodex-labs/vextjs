...
`vext_knowledge_search` returns packaged knowledge, native API examples, and applicability. It distinguishes framework declarations, reviewed knowledge versions, and dependencies resolved separately for service/framework owners. Mismatches and missing packages remain explicit; MCP does not fetch online documentation or connect to databases. See [MCP generation and dependency knowledge](./mcp-generation) for directory overrides, all 17 Recipes, JS/TS contracts, comments, and integration.

`vext_validate_changes` checks content identity, source coverage, logical/physical directories, existing files, duplicates, UTF-8, content digests, syntax, and imports. A shared overlay checks candidate interactions such as route conflicts. Results include file verdicts, directory provenance, and requiredHostSteps; unknown/incomplete evidence cannot produce applyReady. Defaults are overridable, empty directories are not required, and line length or function counts do not establish correctness. The host verifies project formatting and comment conventions.

`vext_project_check` runs bounded static diagnostics for missing optional directories, route response schema gaps, deprecated docs.tags, incomplete service dependency analysis, database cursorSecret configuration choices, missing Redis store targets or Redis targets that depend on unset environment variables, SVG upload review, frontend form/API boundaries, and placeholder tests. The result includes `diagnostics`, `totalBySeverity`, `affectedConsumers`, and `generatedState`; host commands remain host-executed. `vext_runtime_inspect` reads the framework-written `.vext/runtime/snapshots/<instanceId>.json` managed snapshot; MCP does not start services, execute Jobs, or read raw logs; host MCP configuration writes are handled by `vext mcp sync`.
...
Monorepo source ownership is verified through workspace membership, shared declarations, package exports/sourceExports, and consumer dependencies. Adjacent services are not scanned automatically. Explicit external model/locale/frontend sources are read-only when their paths can be proven. Names or shared declarations do not grant arbitrary cross-root candidate writes.
