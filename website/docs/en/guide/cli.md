...
Host sync currently provides the planning layer. `vext mcp sync --root <dir> --check --json` or `vext mcp sync --root <dir> --dry-run --host <id> --json` reads static `dev.mcp.hosts` and returns the Codex, Claude Code, Cursor, VS Code, or Grok config target, stable service key, and launcher digest, but it does not write host configuration files. Real minimal edits, cooperative writes, read-back verification, and host refresh remain part of later sync batches.
