...
Host sync currently supports two modes. `vext mcp sync --root <dir> --check --json` or `--dry-run` only prints a plan. Without those flags, it writes the project `.vext/mcp/launcher.cjs`, `.vext/mcp/hosts.json`, and managed MCP server entries for JSON/JSONC hosts such as Claude Code, Cursor, and VS Code. Codex and Grok TOML config remains plan-only; real TOML minimal edits, cooperative writes, read-back verification, and host refresh remain part of later sync batches.
