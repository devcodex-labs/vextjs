...
`vext_knowledge_search` can search framework capabilities, rules, Recipes, workflows, and major dependency knowledge. Dependency knowledge covers the current versions, Vext entry points, and boundaries for `schema-dsl`, `response-cache-kit`, `cache-hub`, `flex-rate-limit`, `esbuild`, `monsqlize`, `croner`, `ioredis`, and `@modelcontextprotocol/server`; versions come from the installed package's `package.json`.

`vext_validate_changes` checks project identity, directory policy, existing-file overwrites, duplicate paths, encoding, the create-only boundary, and basic generated-code quality. Its response includes each file verdict, the matched directory source, and `requiredHostSteps`. Directory policy comes from the project structure, default Vext directories, and `vext.workspace.json(c)` services/sharedPackages; default directories are recommendations and are not created automatically. Validation blocks common MCP generation drift such as long one-line code, JSON routes without top-level `RouteOptions.responses`, deprecated `docs.tags`, service-local Mongo driver helper types or `app.db` casts, and placeholder tests such as `expect(true).toBe(true)`.

`vext_project_check` runs bounded static diagnostics for missing optional directories, route response schema gaps, deprecated docs.tags, incomplete service dependency analysis, database cursorSecret configuration choices, missing Redis store targets or Redis targets that depend on unset environment variables, SVG upload review, frontend form/API boundaries, and placeholder tests. The result includes `diagnostics`, `totalBySeverity`, `affectedConsumers`, and `generatedState`; host commands remain host-executed. `vext_runtime_inspect` reads the framework-written `.vext/runtime/snapshot.json` managed snapshot; MCP does not start services, execute Jobs, or read raw logs; host MCP configuration writes are handled by `vext mcp sync`.

The `jobs` section of `vext_project_inspect` statically reads `config.jobs` and Job source files, then returns Job names, source files, queue, schedule, payload schema presence, scheduler/worker/store summaries, host-run `vext job ...` commands, and multi-process or cluster deployment notes. MCP does not execute Jobs, connect to queues, or read runtime queue state.

In monorepos, the `services` section expands `vext.workspace.json(c)` service roots, current-root detection, and the shared packages consumed by each service. The `models` section returns the local models directory plus shared packages whose kind is `models`, including sourceExports and consuming services. Candidate validation includes workspace service and shared package directories in the allowed directory policy.

Regular CI explicitly runs the focused MCP unit suite, `npm run verify:mcp`, and `npm run verify:public-surface` so the MCP protocol surface, public catalog, and CLI stdio behavior cannot drift in ordinary PRs. Redis-backed capabilities are covered by a dedicated Redis service lane; local integration tests may skip when Redis is unavailable, but that CI lane must connect to a real Redis service.

Before release, `npm run verify:pack-install` validates the packed installation across ESM/CJS, TypeScript contracts, runtime smoke tests, and an MCP stdio smoke test. The script cleans its temporary install workspace after a successful run and keeps the evidence directory on failure for debugging. Set `VEXT_PREFLIGHT_KEEP_EVIDENCE=1` or `VEXT_PREFLIGHT_WORKSPACE=<dir>` only when a successful run must retain evidence. The real platform, database, browser, and five-host matrix remains part of the release workflow and cannot be replaced by local protocol simulation alone.

Host sync currently supports two modes. `vext mcp sync --root <dir> --check --json` or `--dry-run` only prints a plan. Without those flags, it writes the project `.vext/mcp/launcher.cjs`, `.vext/mcp/hosts.json`, the Codex user-level config (`CODEX_HOME/config.toml` first, otherwise the current user's `.codex/config.toml`), managed MCP server entries for JSON/JSONC hosts such as Claude Code, Cursor, and VS Code, and Vext managed blocks for TOML hosts such as Grok. Apply results include per-target `verified` read-back status. If TOML already contains an unmanaged same-key table, sync returns blocked and preserves the file. When sync writes a launcher, host config, or Skill file, `applied.nextSteps` returns host-specific reload, new-session, and read-only validation guidance. Five-host real runtime validation remains part of the release workflow and cannot be replaced by local file read-back alone.

The optional Skill is bundled with the package. Use `vext mcp skill check` to inspect its manifest, `vext mcp skill print` to print the Markdown, or `vext mcp skill write --output <file>` to write it to a user-selected path. `vext mcp sync --skill` writes the official Skill to the selected host's project-local Skill path. If the target file already exists with different content, sync returns blocked and preserves it.
...
In a monorepo, run commands from the service directory or pass `--root` to `vext dev`. Start, dev, cluster workers, package preloads, and TypeScript diagnostics resolve dependencies from that service, including hoisted installations and pnpm links. Missing framework entry files require rebuilding or reinstalling the resolved framework version. Doctor/typegen source analysis does not require runnable framework artifacts.
...
VextJS requires Node.js **`^20.19.0 || >=22.12.0`**. Use the latest LTS patch of Node.js 22 (at least 22.12.0) or 24, and create a `.node-version` or `.nvmrc` file in the project root directory to specify the version:
