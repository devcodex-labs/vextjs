...
Interactively create new VextJS projects and automatically generate project skeleton and configuration files.
...
Visit `http://localhost:3000` and you should see the framework's JSON response.
...
Compile TypeScript source code into JavaScript to generate a production-usable `dist/` directory; tool products such as typegen and route manifest will be refreshed before building.
...
- Use esbuild for extremely fast compilation (TypeScript → JavaScript)
