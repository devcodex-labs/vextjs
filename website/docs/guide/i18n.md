...
:::warning 子目录模式注意事项
子目录模式不经过 VextJS 的 `i18n-loader`（它只扫描平铺文件）。子目录模式通过 schema-dsl ≥ v1.2.3 的内置递归扫描功能实现：
...
schemaAdapter.configure({
  i18n: path.join(rootDir, 'src/locales'),  // 传路径，schema-dsl 自动递归扫描
});
```

schema-dsl 内置扫描仅支持 `.js` / `.json` 文件，不支持 `.ts`。如需 `.ts` + 子目录，建议编译后运行或在子目录中使用 `.js` 格式。
:::
